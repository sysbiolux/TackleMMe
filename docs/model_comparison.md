...
+ Default: Checking out the visualizations that are generated by default by the pipeline
+ Exploration: Generating additional figures with the pipeline function, for a more guided exploration
...
After looking through all the figures we have a rough first overview over our model.
As a next step the exploratory part beginns where we look more into detail in our metabolic model. 
This more exploratory part might come from the fact that there are certain pathways of interest for which we want to see biological assumpations validated by the model, or just by looking at the heatmaps already generated there might be specific pathways standing out that are showing more differences than others. 

#### Exploration of metabolic models using TackleMMe

Looking at the Fluxsum plots (see Fig. 18) it can be seen that the Citric Acid Cycle has a tendency of increasing in StageI and lower values for the Control and even lower values for the Stage II and StageIV models. In Fig.19 we can also see that this difference can be robustly seen throughout the samples. Using the functions of TackleMMe it is easy to take a closer look into the individual Pathways. 

In a first step we want to get the rxnIDs of all rxns that are part of the pathways of interest:
